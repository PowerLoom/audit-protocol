from .system_ticker_linear import main
import asyncio
import logging
...
async def cleanup_redis(redis_conn: RedisPool):
    ...
    await redis_conn.delete(get_system_ticker_linear_last_epoch())
    logger.info("Deleted key: %s", get_system_ticker_linear_last_epoch())
    await redis_conn.delete(get_system_ticker_linear_epoch_history())
...
async def test_epoch_generation():
    ...
    ticker_process = LinearTickerProcess(name="PowerLoom|EpochTracker|LinearTest1", simulation_mode=True)
    kwargs = dict()
    ...
    epoch_end_block_height = await reader_redis_pool.get(get_system_ticker_linear_last_epoch())
...
async def test_state_resume():
    ...
    ticker_process = LinearTickerProcess(name="PowerLoom|EpochTracker|LinearTest2", simulation_mode=True)
    kwargs = dict()
    await ticker_process.setup(**kwargs)
...
async def test_force_restart_with_redis_state_present():
    ...
    ticker_process = LinearTickerProcess(name="PowerLoom|EpochTracker|LinearTest3", simulation_mode=True)
    kwargs = dict()
    await ticker_process.setup(**kwargs)
